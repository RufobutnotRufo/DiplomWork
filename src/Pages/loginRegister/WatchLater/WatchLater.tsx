...
const WatchLater = () => {
  ...
  const [dataDecriptionToBackround, setDataDecriptionTobackround] = useState<Movie>();


  const getItemDescriptionHover = (movieId:string) =>{
    const element = moviesBase.find((item)=>(
      item.id === movieId
    ))
    ...
    <section className="watch-now-section">
      <div className="watch-now-section-inner-item container">
        {watchNowDatas.map((item) => (
          <div className="watch-now-section-inner-item-img" key={item.imdbid} onMouseEnter={()=> getItemDescriptionHover(item.id)}>
            <img
      ...
      <a href={dataDecriptionToBackround?.imdb_link}>
        <div className="watch-now-section-inner-desc">
          <div className="watch-now-section-inner-desc-poster">
            <img
              className="img"
              src={dataDecriptionToBackround?.big_image}
            />
          </div>
          <div className="block-desc container">
           
            <h1 className="watch-now-section-inner-movie-desc-title">{dataDecriptionToBackround?.title}</h1>
            <h2 className="watch-now-section-inner-movie-desc-rank">{dataDecriptionToBackround?.rank}</h2>
            <h3 className="watch-now-section-inner-movie-desc-rank-imdb">{dataDecriptionToBackround?.year}</h3>
            <ul className="watch-now-section-inner-movie-desc-genre">{dataDecriptionToBackround?.genre.map((item)=>(
              <li>{item}</li>
            ))}</ul>
